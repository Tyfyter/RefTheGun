	...
	public class HeroBow : RefTheItem{
		...
		public override void OnHitEffect(Projectile projectile, NPC target){
			switch ((int)projectile.GetGlobalProjectile<GunGlobalProjectile>().aioverflow[0]){
				case 1:
				Projectile.NewProjectileDirect(projectile.Center, projectile.velocity/2, ModContent.ProjectileType<Combustion>(), projectile.damage, 0, projectile.owner).rotation = (float)Math.Atan2((double)projectile.velocity.X, (double)projectile.velocity.Y);
