	...
	public class RefTheItem : ModItem
	{
        ...
        public override void HoldItem(Player player){
            ...
            if(Main.player[item.owner].GetModPlayer<GunPlayer>().Reloaded){
                ...
                restoredefaults();
                Ammo = /*(int)(*/MaxAmmo/**Main.player[item.owner].GetModPlayer<GunPlayer>().MagMultiply)*/;
        ...
        public override bool CanUseItem(Player player){
            if(player.altFunctionUse == 2 || (Ammo<=0&&MaxAmmo>0&&!Main.player[item.owner].GetModPlayer<GunPlayer>().Reloading&&!Main.player[item.owner].GetModPlayer<GunPlayer>().Reloaded)){
                ...
                if(Ammo >= /*(int)(*/MaxAmmo/**Main.player[item.owner].GetModPlayer<GunPlayer>().MagMultiply)*/)return false;
                SpecialReloadProj(player, Ammo);
                ...
                restoredefaults();
                return (Ammo > 0 || MaxAmmo <= 0) && (!Main.player[item.owner].GetModPlayer<GunPlayer>().Reloading||specialreloadproj);
            }
        }
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack){
            ...
            if((ammoPerMult(0)?Math.Min(Multishot,Ammo):Multishot)*modPlayer.multishotmult==1){
                ...
                if(Main.rand.NextFloat(0,1)<=modPlayer.bulletPoisonChance){
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().passives.Add(1);
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().GlowColor = Color.DarkGreen*0.1f;
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().Color = Color.DarkGreen;
                ...
                if(Main.rand.NextFloat(0,1)<=modPlayer.bulletFreezeChance){
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().passives.Add(2);
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().GlowColor = Color.Aqua*0.1f;
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().Color = Color.Aqua;
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().OverrideColor = true;
                }
                if(Main.rand.NextFloat(0,1)<=modPlayer.bulletBurnChance){
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().passives.Add(3);
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().GlowColor = Color.DarkOrange*0.1f;
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().Color = Color.DarkOrange;
                    Main.projectile[proj].GetGlobalProjectile<GunGlobalProjectile>().OverrideColor = true;
                }
                if(storefiredshots){
                    firedshots.Add(proj);
